...
async function seed() {
    ...
    await Promise.all(getOrders().map((order) => {
        return db.order.create({
            data: order
        });
    }));
    await Promise.all(getPhotos().map((_a) => {
        var { productId } = _a, photoData = __rest(_a, ["productId"]);
        return db.photo.create({
            data: Object.assign(Object.assign({}, photoData), { product: {
                    connect: { id: productId }
                } })
        });
    }));
    await Promise.all(getProductsOnOrders().map((orderProduct) => {
